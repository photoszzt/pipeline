...
class RunState(CommandListState):
    ...
    commandlist = [ (None, 'run:mkdir -p ##TMPDIR##/out_0/')
                  , ('OK:RETVAL(0)', 'run:./ffmpeg -start_number 1 -i ##TMPDIR##/in_0/%08d.png '
                                     '-s 800x800 -sws_flags neighbor -c:a copy -safe 0 -start_number 1 ##TMPDIR##/out_0/%08d.png')
                  , ('OK:RETVAL(0)', None)
                    ]
...
class InitState(InitStateTemplate):
    ...
    def __init__(self, prevState, **kwargs):
        ...
        self.trace_func = lambda ev, msg, op: default_trace_func(ev, msg, op, stage='pixscale')
